...
RUN go build ./cmd/git-events-publisher

# Let's create a /dist folder containing just the files necessary for runtime.
# Later, it will be copied as the / (root) of the output image.
WORKDIR /dist
RUN cp /build/git-events-publisher ./git-events-publisher
...
RUN ldd git-events-publisher | tr -s '[:blank:]' '\n' | grep '^/' | \
...
ENTRYPOINT ["/git-events-publisher"]